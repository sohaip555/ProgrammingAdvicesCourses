...
void DeleteCleint(vector <stClientData> ClientsData, string AccountNumber)
{
    fstream MyFile;
...
void ShowCleintData()
{
    ...
    if (FindCleintData(ClientsData, AccountNumber))
    {
        ...
        cout << "\nAre you sure you want delete this cleint Y/N ?";
        cin >> Delete;
        if (toupper(Delete) == 'Y')
        {
            DeleteCleint(ClientsData, AccountNumber);
